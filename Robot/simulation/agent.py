...
class ActorNetwork(nn.Module):
    ...
    def load_checkpoint(self):
        ...
        # The bundled policy predates the final observation feature.  Preserve
        # its learned weights and give that new feature an initially neutral
        # (zero) connection instead of rejecting an otherwise valid checkpoint.
        weight_key = 'actor.0.weight'
        expected = self.state_dict()[weight_key].shape
        loaded = state_dict[weight_key]
        if loaded.shape[0] == expected[0] and loaded.shape[1] + 1 == expected[1]:
            state_dict[weight_key] = T.nn.functional.pad(loaded, (0, 1))
