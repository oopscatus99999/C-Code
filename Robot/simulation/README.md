...
The default mode. Loads the included weights from `models/` and runs the simulation visually.
...
Training runs for `TOTAL_STEPS = 1_000_000` environment steps. Model weights are saved every 10 episodes to `models/`. Progress is logged to `reward_log.txt`.
